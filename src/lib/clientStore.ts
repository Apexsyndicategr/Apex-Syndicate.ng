  ...
  return {
      ...
      devUpdates: [
        {
          ...
          images: [
            '/images/apex_editor_demo_screenshot.jpg',
          ],
    ...
    products: [
      ...
      },
      {
        ...
        whatsNew: ['In active development! Official release date and specifications are To Be Determined (TBD).'],
      },
    ],
    requests: [],
    notifications: [],
...
export function loadClientData(): StoreData {
  try {
    ...
    // Ensure Apex XI 2027 is purged if present in localStorage
    if (parsed.products.some((p: Product) => p.id === 'apex-xi-2027')) {
      parsed.products = parsed.products.filter((p: Product) => p.id !== 'apex-xi-2027');
